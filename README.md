...
回复带「智能体 · X」标识；建好的图表/看板可一键跳转。服务独立（`:8004`）；DeepSeek 不可用时降级。
...
**智能助手（多智能体）**：右上角「智能助手」对话。按意图自动分派：**查数据**（MCP 工具）、**建图表/看板**（如「做一个电商运营看板」→ 自动建好并给跳转链接）、**发布后台任务**、**通用答疑**。需在 `.env` 配 `DEEPSEEK_API_KEY`。
