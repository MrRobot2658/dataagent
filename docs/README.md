...
Quasar 定位：**多数据源计算存储智能代理——为 Agent 提供数据底座**。统一接入多源数据、自动清洗、灵活转化、高性能计算，向上对 Agent 暴露结构化数据接口（OneID 画像 / 圈人 / 聚合指标）。支持**计算存储分离**（计算弹性扩缩 + Doris 存算分离）或**一体化部署**（单机开箱即用），默认存储引擎 **Apache Doris**。

按 Twilio Segment 的信息架构，把平台拆成 **1 个平台底座 + 9 个业务模块**，并在其上扩展 **3 个 Quasar 一级菜单（知识库 / 应用 / 分析）**。每个模块一份文档，含 **详细设计（产品）/ 技术设计 / TODOs**，可被独立认领、并行开发。文档已**直接放在 `docs/` 下**（不再分 `modules/` 子目录）。
...
| 10 | 知识库 Knowledge | —（Quasar 扩展） | 真实 | [10-knowledge.md](./10-knowledge.md) |
